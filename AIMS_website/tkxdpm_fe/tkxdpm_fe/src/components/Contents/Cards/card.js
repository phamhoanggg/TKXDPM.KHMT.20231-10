...
export default function Card({mediaList,state,setState,keyword}) {
    ...
    let mediaCate = mediaList.filter(e => e.idOfCategory === state);
    if(state ==='all')
    mediaCate = mediaList;
    else if(state ==='popular'){
        mediaCate = mediaList.filter(e => e.isBestSeller === true);
    }

    if (keyword !== ''){
        mediaCate = mediaList.filter(e => e.name.toLowerCase().includes(keyword) || e.productDescription.toLowerCase().includes(keyword) || e.productDetails.toLowerCase().includes(keyword));
    }
        ...
        <div className="pt-20 flex flex-col lg:flex-row flex-wrap gap-x-8 gap-y-20 justify-center items-center">

            {
                mediaCate.map((media)=>(
                    ...
                    <div key ={media.id} class="basis-1/5 rounded-lg flex flex-col xl:flex-row items-center justify-center bg-gray-100 
                                w-52 h-64 md:max-w-xl md:max-h-xl transition ease-in-out hover:-translate-y-1 hover:scale-110 shadow-lg hover:bg-gray-200"
                        onClick={()=>{
                            navigate(`/Product/${media.id}`)
                        }}
                        >

                        <img className="w-24 pb-64 rounded-t-lg absolute" src={media.image} alt="product" />
                        
                        <div className="px-5 pb-5 pt-20 ">
            
                            <h3 className="text-xl font-semibold tracking-tight text-gray-900 ">{media.name}</h3>
                            <p className="text-sm text-gray-600">{media.dishDetails}</p>
                        ...
                        <div className="flex justify-between items-center">
                            <span className='text-3xl font-bold text-gray-900'>{media.price}.000 VNĐ</span>
                            {checkInCart(media.id) && <AddButton></AddButton> ?<CheckButton></CheckButton>:<AddButton></AddButton>}
